...
<template>
  <NModal v-model:show="visible" preset="dialog" :title="title">
    <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" label-width="auto"
      require-mark-placement="right-hanging" size="medium" class="max-w-150">
      <NFormItem :label="$t('page.manage.user.userName')" path="userName">
        <NInput v-model:value="model.userName" :placeholder="$t('page.manage.user.form.userName')" :on-update-value="(value: string) => {
      ...
      <NFormItem :label="$t('page.manage.user.userRole')" path="roles">
        <NSelect v-model:value="model.byUserRoleCodeList" multiple filterable clearable :options="roleOptions"
          :placeholder="$t('page.manage.user.form.userRole')" />
      </NFormItem>
      <NFormItem :label="$t('page.manage.common.statusType._')" path="status">
